...
import { GoIcon, StopIcon } from './components/Icons';
...
function App() {
  ...
  const executionFuncs = useRef<{
    executeScriptFrom: (startBlock: ScriptBlock, spriteId: string, allBlocks: ScriptBlock[]) => Promise<void>;
    triggerEvent: (eventName: BlockName, options?: { spriteId?: string; message?: string }) => Promise<void>;
    executeBlock: (block: ScriptBlock, spriteId: string, currentScript: ScriptBlock[]) => Promise<void>;
  }>({ executeScriptFrom: async () => {}, triggerEvent: async () => {}, executeBlock: async () => {} });
  ...
  const paletteBlocks = useMemo(() => {
    ...
    const cBlockBodyHeights = useMemo(() => {
        ...
        // Helper function to get the height of a single block. It needs to look up
        // the body height of C-blocks from our `heights` object as it's being built.
        const getBlockHeight = (block: ScriptBlock): number => {
            if (block.shape === 'c_block') {
                // If we have a calculated height, use it. Otherwise, estimate. This breaks recursion.
                const bodyHeight = heights[block.id] ?? C_BLOCK_MIN_BODY_HEIGHT;
                return C_BLOCK_HEADER_HEIGHT + bodyHeight + C_BLOCK_BOTTOM_HEIGHT;
            }
            return block.shape === 'hat' ? HAT_BLOCK_HEIGHT : BLOCK_HEIGHT;
        };
    
        // We may need to iterate a couple of times for nested C-block heights to propagate.
        // Two passes are usually enough for simple nesting.
        for (let i = 0; i < 2; i++) {
            for (const spriteId in scripts) {
                const currentScript = scripts[spriteId] || [];
                const cBlocks = currentScript.filter(b => b.shape === 'c_block');
                
                for (const cBlock of cBlocks) {
                    const childStack = findChildStack(cBlock, currentScript);
                    let childrenHeight = 0;
                    for (const child of childStack) {
                        childrenHeight += getBlockHeight(child);
                    }
    ...
    const alignScript = useCallback((currentScript: ScriptBlock[]): ScriptBlock[] => {
        if (!currentScript || currentScript.length === 0) return [];

        const scriptMap = new Map(currentScript.map(b => [b.id, { ...b }]));
        ...
        const allBlocks = [...scriptMap.values()];
        const potentialConnections = new Map<string, string>(); // childId -> parentId

        allBlocks.forEach(parent => {
            if (parent.shape === 'reporter') return;

            const parentHeight = getBlockHeight(parent);

            // Find next block
            const nextY = parent.y + parentHeight;
            const nextBlock = allBlocks.find(child =>
                child.id !== parent.id &&
                (child.shape === 'stack' || child.shape === 'c_block') &&
                Math.abs(child.x - parent.x) < 20 &&
                Math.abs(child.y - nextY) < 20
            );
            if (nextBlock) potentialConnections.set(nextBlock.id, parent.id);

            // Find first child in C-Block
            if (parent.shape === 'c_block') {
                const childY = parent.y + C_BLOCK_HEADER_HEIGHT;
                const childX = parent.x + C_BLOCK_INDENT;
                const firstChild = allBlocks.find(child =>
                    child.id !== parent.id &&
                    (child.shape === 'stack' || child.shape === 'c_block') &&
                    Math.abs(child.x - childX) < 20 &&
                    Math.abs(child.y - childY) < 20
                );
                if (firstChild) potentialConnections.set(firstChild.id, parent.id);
            }
        });

        const heads = allBlocks.filter(b => !potentialConnections.has(b.id));

        const alignStack = (head: ScriptBlock) => {
            if (processed.has(head.id)) return;
            processed.add(head.id);

            let current = scriptMap.get(head.id);
            if (!current) return;
            ...
            while (cursor) {
                ...
                const parentHeight = getBlockHeight(parent);
                const nextY = parent.y + parentHeight;
                const nextBlock = allBlocks.find(child => potentialConnections.get(child.id) === parent.id && Math.abs(child.y - nextY) < 20);
                
                if (nextBlock) {
                    const child = scriptMap.get(nextBlock.id);
            ...
            if (current.shape === 'c_block') {
                const childY = current.y + C_BLOCK_HEADER_HEIGHT;
                const childX = current.x + C_BLOCK_INDENT;
                const firstChildBlock = allBlocks.find(child => potentialConnections.get(child.id) === current.id && Math.abs(child.y - childY) < 20);

                if (firstChildBlock) {
                     const child = scriptMap.get(firstChildBlock.id);
                     if(child && !processed.has(child.id)) {
                        child.x = childX;
                        child.y = childY;
                        processed.add(child.id);
        ...
        heads.forEach(head => alignStack(head));
        return [...scriptMap.values()];
    }, [cBlockBodyHeights]);


    const handleScriptChange = useCallback((updater: (prev: ScriptBlock[]) => ScriptBlock[]) => {
        if (!activeSpriteId) return;

        setScripts(prevScripts => {
            const current = prevScripts[activeSpriteId] || [];
            const updated = updater(current);
            const aligned = alignScript(updated);
            return {
                ...prevScripts,
                [activeSpriteId]: aligned,
            };
        });
    }, [activeSpriteId, alignScript]);
  ...
  const isTruthy = useCallback((val: any): boolean => {
  ...
  const resolveParam = useCallback(async (paramValue: ParamValue, spriteId: string, currentScript: ScriptBlock[]): Promise<string | number> => {
  ...
  const executeReporterBlock = useCallback(async (block: ScriptBlock, spriteId: string, currentScript: ScriptBlock[]): Promise<string | number> => {
      ...
      return '';
  }, [variables, resolveParam, isTruthy]);


  const findNextBlock = useCallback((currentBlock: ScriptBlock, allBlocks: ScriptBlock[]): ScriptBlock | undefined => {
      if (currentBlock.shape === 'reporter') return undefined;
      const currentHeight = currentBlock.shape === 'c_block' 
          ? C_BLOCK_HEADER_HEIGHT + (cBlockBodyHeights[currentBlock.id] || C_BLOCK_MIN_BODY_HEIGHT) + C_BLOCK_BOTTOM_HEIGHT
          : (currentBlock.shape === 'hat' ? HAT_BLOCK_HEIGHT : BLOCK_HEIGHT);

      const nextY = currentBlock.y + currentHeight;

      return allBlocks.find(block => 
          block.id !== currentBlock.id &&
          (block.shape === 'stack' || block.shape === 'c_block') &&
          Math.abs(block.x - currentBlock.x) < 20 && 
          Math.abs(block.y - nextY) < 20 
      );
  }, [cBlockBodyHeights]);

  const findFirstChildBlock = useCallback((parentBlock: ScriptBlock, allBlocks: ScriptBlock[]): ScriptBlock | undefined => {
      if (parentBlock.shape !== 'c_block') return undefined;
      
      const childY = parentBlock.y + C_BLOCK_HEADER_HEIGHT;
      const childX = parentBlock.x + C_BLOCK_INDENT;

      return allBlocks.find(block => 
          block.id !== parentBlock.id &&
          (block.shape === 'stack' || block.shape === 'c_block') &&
          Math.abs(block.x - childX) < 10 &&
            ...
            if (
                ...
                if (draggedBlockInfo.isNew) {
                    ...
                    if (foundTemplate) {
                        ...
                        handleScriptChange(prev => [...prev, newBlock]);
                    }
                    ...
                    handleScriptChange(currentScript => currentScript.map(b => {
                ...
                if (!draggedBlockInfo.isNew) {
                    ...
                    handleScriptChange(script => script.filter(b => !idsToRemove.has(b.id)));
                }
            }
    ...
    }, [draggedBlockInfo, handleScriptChange, variables, definedScripts, getStack]);


    const executeScriptFrom = useCallback(async (startBlock: ScriptBlock, spriteId: string, allBlocks: ScriptBlock[]) => {
        let currentBlock: ScriptBlock | undefined = startBlock;
        while (currentBlock && executionRef.current.running) {
            await executionFuncs.current.executeBlock(currentBlock, spriteId, allBlocks);
            if (!executionRef.current.running) break;
            currentBlock = findNextBlock(currentBlock, allBlocks);
        }
    }, [findNextBlock]);

    const triggerEvent = useCallback(async (eventName: BlockName, options: { spriteId?: string; message?: string } = {}) => {
        const promises: Promise<void>[] = [];

        for (const sprite of sprites) {
            const spriteScript = scripts[sprite.id] || [];
            let hatBlocks: ScriptBlock[] = [];

            if (eventName === BlockName.EVENTS_WHEN_FLAG_CLICKED) {
                hatBlocks = spriteScript.filter(b => b.name === BlockName.EVENTS_WHEN_FLAG_CLICKED);
            } else if (eventName === BlockName.EVENTS_WHEN_SPRITE_CLICKED && options.spriteId === sprite.id) {
                hatBlocks = spriteScript.filter(b => b.name === BlockName.EVENTS_WHEN_SPRITE_CLICKED);
            } else if (eventName === BlockName.EVENTS_WHEN_I_RECEIVE) {
                hatBlocks = spriteScript.filter(b => 
                    b.name === BlockName.EVENTS_WHEN_I_RECEIVE && 
                    b.params.message === options.message
                );
            }

            for (const headBlock of hatBlocks) {
                const firstBlock = findNextBlock(headBlock, spriteScript);
                if (firstBlock) {
                    promises.push(executeScriptFrom(firstBlock, sprite.id, spriteScript));
                }
            }
        }
        
        // Let all triggered scripts run in parallel, but don't block the caller.
        Promise.all(promises).catch(err => console.error("Error during event execution:", err));
    }, [sprites, scripts, findNextBlock, executeScriptFrom]);


    const executeBlock = useCallback(async (block: ScriptBlock, spriteId: string, currentScript: ScriptBlock[]) => {
        ...
        switch (block.name) {
            ...
            case BlockName.SOUND_STOP_ALL:
                stopAllSounds();
                break;
            case BlockName.EVENTS_BROADCAST:
                const broadcastMessage = String(await resolveParam(params.message, spriteId, currentScript));
                executionFuncs.current.triggerEvent(BlockName.EVENTS_WHEN_I_RECEIVE, { message: broadcastMessage });
                break;
            ...
            case BlockName.THREED_CREATE_OBJECT:
                ...
                break;
            // Control blocks
            case BlockName.CONTROL_WAIT:
                const waitSeconds = Number(await resolveParam(params.seconds, spriteId, currentScript));
                await new Promise(resolve => setTimeout(resolve, waitSeconds * 1000));
                break;
            case BlockName.CONTROL_REPEAT:
                const times = Math.round(Number(await resolveParam(params.times, spriteId, currentScript)));
                const firstChildRepeat = findFirstChildBlock(block, currentScript);
                if (firstChildRepeat) {
                    for (let i = 0; i < times; i++) {
                        if (!executionRef.current.running) break;
                        await executionFuncs.current.executeScriptFrom(firstChildRepeat, spriteId, currentScript);
                        // Yield to allow UI updates in the loop
                        await new Promise(resolve => setTimeout(resolve, 16));
                    }
                }
                break;
            case BlockName.CONTROL_FOREVER:
                const firstChildForever = findFirstChildBlock(block, currentScript);
                if (firstChildForever) {
                    while (executionRef.current.running) {
                        await executionFuncs.current.executeScriptFrom(firstChildForever, spriteId, currentScript);
                        // Yield to prevent browser freeze and allow UI updates
                        await new Promise(resolve => setTimeout(resolve, 16));
                    }
                }
                break;
            case BlockName.CONTROL_IF:
                const condition = await resolveParam(params.condition, spriteId, currentScript);
                if (isTruthy(condition)) {
                    const firstChildIf = findFirstChildBlock(block, currentScript);
                    if (firstChildIf) {
                        await executionFuncs.current.executeScriptFrom(firstChildIf, spriteId, currentScript);
                    }
                }
                break;
            case BlockName.CONTROL_WAIT_UNTIL:
                while (executionRef.current.running) {
                    const conditionWait = await resolveParam(params.condition, spriteId, currentScript);
                    if (isTruthy(conditionWait)) {
                        break;
                    }
                    await new Promise(resolve => setTimeout(resolve, 16)); // Check every frame
                }
                break;
        }
    }, [resolveParam, setSprites, playSound, stopAllSounds, setAskingState, setVariables, isTruthy, findFirstChildBlock, setThreeDObjects, setIsGenerating, executeScriptFrom]);

    useEffect(() => {
      executionFuncs.current = { executeBlock, executeScriptFrom, triggerEvent };
    }, [executeBlock, executeScriptFrom, triggerEvent]);


    const handleSpriteClick = useCallback((spriteId: string) => {
        if (!isRunning) {
            // Initialize run state if not already running
            setIsRunning(true);
        ...
        executionFuncs.current.triggerEvent(BlockName.EVENTS_WHEN_SPRITE_CLICKED, { spriteId });
    }, [isRunning]);

  const runScript = async () => {
    // If running, stop and restart (Scratch behavior)
    if (isRunning) {
        stopScript();
    ...
    executionFuncs.current.triggerEvent(BlockName.EVENTS_WHEN_FLAG_CLICKED);
  };
  ...
  const TabButton: React.FC<{
      ...
      return (
          <button
              ...
              className={`px-4 py-2 text-sm font-bold rounded-t-lg border-b-0 ${isActive ? 'bg-panel-light text-electric-yellow' : 'bg-panel-dark text-slate-300 hover:bg-slate-700'}`}
          >
              {label}
          </button>
      )
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden text-sm">
      ...
      <header className="bg-panel-dark flex-shrink-0 flex items-center justify-between p-2 shadow-lg z-20">
        ...
        <div className="flex items-center gap-2">
          <button onClick={runScript} className="bg-green-600 hover:bg-green-500 disabled:bg-slate-500 disabled:cursor-not-allowed text-white p-2 rounded-full transition-colors flex items-center justify-center" aria-label="Start script">
            <GoIcon />
          </button>
          <button onClick={stopScript} disabled={!isRunning} className="bg-red-600 hover:bg-red-500 disabled:bg-slate-500 disabled:cursor-not-allowed text-white p-2 rounded-full transition-colors flex items-center justify-center" aria-label="Stop script">
            <StopIcon />
          </button>
        </div>
      </header>
      ...
      <main className="flex flex-1 overflow-hidden p-4 gap-4">
        ...
        <div className="flex-1 flex flex-col gap-0">
          ...
          <div className="flex-1 bg-panel-light rounded-b-lg rounded-tr-lg border border-slate-700 overflow-hidden">
            {activeTab === 'code' && (
              <ScriptArea
        ...
        <div className="w-1/3 max-w-md flex-shrink-0 flex flex-col gap-4">
            <Stage 
                sprites={sprites} 
                onReset={resetStage}
                variables={variables}
            ...
            />
            <div className="flex-1 bg-panel-dark rounded-lg border border-slate-700 overflow-hidden flex flex-col">
              {activeSprite && (
                  <SpriteEditor 
                      sprite={activeSprite} 
